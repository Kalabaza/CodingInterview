...
bool HasUniqueChars(const std::string &text) {
    ...
    /// Considering that only ASCII characters are going to be received on the string, an array of 256 positions is
    /// enough to tell if there are any repeated characters in the input string. An array of booleans will be used and
    /// it will be initialized to false in all the positions.
    std::array<bool, 256> charArray{};

    /// Take each letter of the input text and mark the position on the array for that specific letter, if the position
    /// was already marked it means that a duplicated character was found.
